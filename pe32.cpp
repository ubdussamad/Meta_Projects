...
int chk_ext(long int num) { // Returns 1 if it's present else 0
  for ( int i = 0; i < ci+1 ; i++ ) {
    if ( buffer[i] == num ) { return(1); }
  }
  return(0);
}
...
int check_pan_digital(long int num1,long int num2,long int num3) {
  for ( int i = 1; i < 10; i++ ) {
  if(chk_digit(num1,i) ==0&&chk_digit(num2,i)==0&&chk_digit(num3,i)==0)
  {return(0);}}return(1);}

int chk_digit(long int num, int target) {
  while (num > 10) {if (target == num%10) { return(1); }
  num = num/10;}if (target != num){return(0);}return(1);}
